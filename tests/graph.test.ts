...
describe('buildSiteIndex', () => {
  ...
  it('reports missing link targets without blocking the index', () => {
    const result = buildSiteIndex([page({ id: 'a', outgoing: ['b', 'missing'] }), page({ id: 'b' })], routing, '/typwiki');
  ...
  it('rejects duplicate page IDs from different files', () => {
    expect(() =>
      buildSiteIndex(
        [page({ file: 'drafts/first.typ', id: 'stable-id' }), page({ file: 'archive/second.typ', id: 'stable-id' })],
        routing,
        '/typwiki',
      ),
    ).toThrow(TypwikiError);
  });
  ...
  it('rejects navigation entries that set both id and href', () => {
    expect(() => buildSiteIndex([page({ id: 'home' })], routing, '/typwiki', [{ id: 'home', href: '/x', label: 'X' }])).toThrow(
      /cannot set both id and href/,
    );
  });

  it('rejects navigation entries that set neither id nor href', () => {
    expect(() => buildSiteIndex([page({ id: 'home' })], routing, '/typwiki', [{ label: 'X' }])).toThrow(/must set id or href/);
  });
});
