...
import { escapeDiagnostic, injectReloadScript } from './server/reload-client.js';
...
export async function startServer(config: TypwikiConfig): Promise<void> {
  ...
  app.get('/', async (_request, reply) => {
    ...
    const diagnostics = lastError ? `<pre>${escapeDiagnostic(lastError)}</pre>` : '';
  ...
  const watcher = chokidar.watch([join(config.root, config.pagesDir), join(config.root, config.libDir), join(config.root, 'assets')], {
    ignoreInitial: true,
  });
  watcher.on('all', () => {
